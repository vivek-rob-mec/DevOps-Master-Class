# MERN Team Collaboration Platform

Production-oriented **Full-stack two-tier application** capstone using **MongoDB / Express 5 / Node.js 24 LTS** with **React 19 / Vite 8**.
...
|---|---|---:|---|
| `api` | Express API, task workflow rules, MongoDB adapters, and metrics | 4000 | `/health` |
| `frontend` | React collaboration board and same-origin API proxy | 8080 | `/health` |
...
Open `http://localhost:8089`. Stop with `docker compose down`; add `--volumes` only for disposable local data.
...
docker compose build
docker compose up -d
bash scripts/smoke-test.sh http://localhost:8089
docker compose down
```
...
helm lint helm
helm template collaboration helm --namespace collaboration-mern
