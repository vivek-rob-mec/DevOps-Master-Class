# .NET Insurance Claims Modular Monolith

Production-oriented **Modular monolith** capstone using **.NET 10 LTS / ASP.NET Core / PostgreSQL** with **ASP.NET static web application**.
...
Open `http://localhost:8092`. Stop with `docker compose down`; add `--volumes` only for disposable local data.
...
docker compose build
docker compose up -d
bash scripts/smoke-test.sh http://localhost:8092
docker compose down
```
...
helm lint helm
helm template claims helm --namespace claims-dotnet
