# Python Commerce Microservices Platform

Production-oriented **Microservices** capstone using **Python 3.12 / FastAPI** with **React / Vite**.
...
|---|---|---:|---|
| `gateway` | Public API, validation, correlation, routing | 8000 | `/health` |
| `catalog` | Product catalog ownership | 8001 | `/health` |
| `orders` | Idempotent order lifecycle | 8002 | `/health` |
| `frontend` | React customer experience | 8080 | `/health` |
...
Open `http://localhost:8081`. Stop with `docker compose down`; add `--volumes` only for disposable local data.
...
helm lint helm
helm template commerce helm --namespace commerce-python
