# Python Learning Management Modular Monolith

Production-oriented **Modular monolith** capstone using **Python 3.12 / FastAPI / PostgreSQL** with **Server-rendered HTML / progressive JavaScript**.
...
|---|---|---:|---|
| `app` | Learning UI, catalog/enrollment/progress modules, API, and persistence adapters in one deployable unit | 8000 | `/health` |
...
Open `http://localhost:8087`. Stop with `docker compose down`; add `--volumes` only for disposable local data.
...
docker compose build
docker compose up -d
bash scripts/smoke-test.sh http://localhost:8087
docker compose down
```
...
helm lint helm
helm template learning helm --namespace learning-python
