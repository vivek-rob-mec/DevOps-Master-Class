# Node.js Helpdesk Modular Monolith

Production-oriented **Modular monolith** capstone using **Node.js 24 LTS / Express 5 / PostgreSQL** with **React 19 / Vite 8**.
...
|---|---|---:|---|
| `app` | React UI, ticket/SLA/audit modules, HTTP API, and persistence adapters in one deployable unit | 3000 | `/health` |
...
Open `http://localhost:8086`. Stop with `docker compose down`; add `--volumes` only for disposable local data.
...
docker compose build
docker compose up -d
bash scripts/smoke-test.sh http://localhost:8086
docker compose down
```
...
helm lint helm
helm template helpdesk helm --namespace helpdesk-node
