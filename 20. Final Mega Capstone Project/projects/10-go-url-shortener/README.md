# Go Cloud-Native URL Shortener

Production-oriented **Cloud-native service** capstone using **Go 1.25 / net/http / pgx / PostgreSQL** with **Embedded responsive web interface**.
...
Open `http://localhost:8090`. Stop with `docker compose down`; add `--volumes` only for disposable local data.
...
docker compose build
docker compose up -d
bash scripts/smoke-test.sh http://localhost:8090
docker compose down
```
...
helm lint helm
helm template shortener helm --namespace shortener-go
