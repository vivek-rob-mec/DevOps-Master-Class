# PHP Inventory Management Modular Monolith

Production-oriented **Modular monolith** capstone using **PHP 8.3 / Slim / PostgreSQL** with **Vue 3 / Vite**.
...
|---|---|---:|---|
| `app` | Vue UI, HTTP API, inventory domain, and persistence adapters in one deployable unit | 8080 | `/health` |
...
Open `http://localhost:8084`. Stop with `docker compose down`; add `--volumes` only for disposable local data.
...
docker compose build
docker compose up -d
bash scripts/smoke-test.sh http://localhost:8084
docker compose down
```
...
helm lint helm
helm template inventory helm --namespace inventory-php
