# Rust Payment Risk and Transactional Outbox Service

Production-oriented **Event-driven cloud service** capstone using **Rust 1.97 / Axum / Tokio / PostgreSQL / NATS JetStream** with **HTTP API**.
...
|---|---|---:|---|
| `app` | Axum API, deterministic risk rules, PostgreSQL ledger, and transactional outbox publisher | 8080 | `/health` |
...
Open `http://localhost:8097`. Stop with `docker compose down`; add `--volumes` only for disposable local data.
...
docker compose build
docker compose up -d
bash scripts/smoke-test.sh http://localhost:8097
docker compose down
```
...
helm lint helm
helm template payment-risk helm --namespace payment-risk-rust
