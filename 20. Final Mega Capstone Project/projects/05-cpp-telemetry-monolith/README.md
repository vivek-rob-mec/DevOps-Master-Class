# C++ Edge Telemetry Modular Monolith

Production-oriented **Modular monolith** capstone using **C++20 / POSIX HTTP / SQLite** with **React / Vite**.
...
|---|---|---:|---|
| `app` | React UI, telemetry HTTP API, domain rules, and SQLite adapter in one deployable unit | 8080 | `/health` |
...
Open `http://localhost:8085`. Stop with `docker compose down`; add `--volumes` only for disposable local data.
...
docker compose build
docker compose up -d
bash scripts/smoke-test.sh http://localhost:8085
docker compose down
```
...
helm lint helm
helm template telemetry helm --namespace telemetry-cpp
